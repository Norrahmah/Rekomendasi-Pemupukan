...
<div class="card border-0 border-4 mt-5">
    <div class="card-body p-5">
        <div class="card-title d-flex align-items-center">
            <h5 class="mb-0 text-black">Form Rekomendasi Pertumbuhan</h5>
        </div>
        <hr>
        <form id="rekomendasiForm" action="{{ route('rekomendasi-tumbuh.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="usia_cabai_id" name="usia_cabai_id" required>
                    ...
                    @foreach ($usiaCabai as $usia)
                        <option value="{{ $usia->id }}">{{ $usia->keterangan }} ( {{ $usia->usia }} )</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <p class="text-muted small">Pilih tingkat keasaman (pH) tanah Anda:</p>
                    ...
                    @foreach ($karakteristikTanaman as $karakteristik)
                        <div class="col-md-4 mb-3"> 
                            <div class="card karakteristik-tanaman-card" data-id="{{ $karakteristik->id }}">
                                <div class="card-img-wrapper"> <img src="{{ url('/files/' . $karakteristik->gambar) }}" class="card-img-top" alt="Gambar Karakteristik Tanaman"> </div>
                                <div class="card-body"> 
            ...
            <button type="submit" class="btn btn-primary"><strong>Dapatkan Rekomendasi</strong></button>
        </form>
    </div>
</div>
...
<script>
    ...
    document.querySelectorAll('.karakteristik-tanaman-card').forEach(card => {
        card.addEventListener('click', function() {
            // Update the hidden input with the selected card's ID
...
<style>
    ...
    .card-img-wrapper {
    overflow: hidden;
    height: 260px; /* Reduced the height */
    }
    .karakteristik-tanaman-card.selected {
    border-color: #0d6efd; 
    background-color: #0d6efd;  /* Darker blue for background */
